	...
	fn process(&mut self, cfg: &GraphConfig, data: &Matrix<f64>) -> Vec<DataSet> {
		...
		for (n, chan_queue) in self.buf.iter().enumerate().rev() {
			...
			if chunk.len() < sample_len as usize {
				// Not enough data yet, pad with zeros or skip processing
				chunk.resize(sample_len as usize, 0.0);
			} else if chunk.len() > sample_len as usize {
				// Too much data, truncate to expected size
				chunk.truncate(sample_len as usize);
			}
			if self.window {
			...
			let fft = planner.plan_fft_forward(tmp.len());
